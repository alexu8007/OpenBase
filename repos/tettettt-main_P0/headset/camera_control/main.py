...
def check_status():
    try:
        ...
        service_status = subprocess.run(['/usr/bin/systemctl', 'is-active', SERVICE_NAME], 
...
@check_auth
def start_service():
    result = subprocess.run(['/usr/bin/systemctl', 'start', SERVICE_NAME], 
                          capture_output=True, text=True, shell=False)
...
@check_auth
def stop_service():
    result = subprocess.run(['/usr/bin/systemctl', 'stop', SERVICE_NAME], 
                          capture_output=True, text=True, shell=False)
...
@check_auth
def restart_service():
    result = subprocess.run(['/usr/bin/systemctl', 'restart', SERVICE_NAME], 
                          capture_output=True, text=True, shell=False)
